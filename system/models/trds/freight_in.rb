...
module Trds
  class FreightIn
    ...
    belongs_to  :freight,  class_name: 'Trds::Freight',     inverse_of: :ins
    belongs_to  :doc_grn,  class_name: 'Trds::Grn',         inverse_of: :freights
    ...
    class << self
      ...
      # @todo
      def pos(s)
        where(:doc_grn_id.in => Trds::Grn.where(unit_id: PartnerFirm.pos(s).id).pluck(:id))
      end
    end # Class methods
