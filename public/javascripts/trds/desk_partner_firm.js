    ...
    $.extend(true, Trds, {
      desk: {
        partner_firm: {
            ...
            btns.each(function() {
              ...
              if ($bd.action === 'print') {
                return $button.on('click', function() {
                  ...
                  $url = "/sys/trds/report/print?rb=yearly_stats";
                  if ($bd.fn) {
                    $url += "&fn=" + $bd.fn;
                  }
                  if ($bd.uid) {
                ...
                $button.on('click', function() {
                  ...
                  if ($('input.param.doc_ary').val().split(',')[0] === "" || $('input.param.doc_ary').val().split(',').length < $('input:checkbox').length) {
                    ...
                    Trds.desk.partner_firm.doc_ary = [];
                  }
                  $('input.param.doc_ary').val(Trds.desk.partner_firm.doc_ary);
                  $params = jQuery.param($('.param').serializeArray());
                  $url = "/sys/trds/partner_firm/query?" + $params;
                  Trst.desk.init($url);
                });
          ...
          init: function() {
            this.doc_ary = [];
            Trds.desk.partner_firm.buttons($('button'));
            Trds.desk.partner_firm.selects($('select.param,input.select2'));
            Trds.desk.partner_firm.inputs($('input.doc_ary'));
            return $log('Trds.desk.partner_firm.init() OK...');
          }
        }
      }
    });
    return Trds.desk.partner_firm;
  });
